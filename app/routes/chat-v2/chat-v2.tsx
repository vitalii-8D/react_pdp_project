...
export default function ChatV2({ loaderData, actionData }: Route.ComponentProps) {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-3xl font-black tracking-tight text-slate-900">Chat V2</h1>
        <p className="text-slate-500 mt-1">
          Same rooms and messages as Chat, but real-time updates run over GraphQL subscriptions instead of
          Socket.IO.
        </p>
      </div>
      ...
      {rooms.length === 0 ? (
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          {rooms.map((room) => (
            <Link key={room.id} to={paths.chatV2Room(room.id)}>
      ...
      <div className="space-y-4">
        ...
        <UserSearch searchAction={paths.chatV2UsersSearch()} startDmAction={paths.chatV2StartDm()} />
      ...
      {isAdmin && (
        <Card className="p-6 space-y-4">
          ...
          <Form method="post" className="space-y-4">
            <TextField id="chat-v2-room-name" name={ChatFormField.Name} label="Name" required />
            <TextField id="chat-v2-room-description" name={ChatFormField.Description} label="Description (optional)" />
