...
import { createPostMutation } from "../lib/graphql/posts.server";
...
export async function action({ request }: Route.ActionArgs) {
  ...
  try {
    await createPostMutation(token, { title, content, slug, published, categoryIds, metadata });
    return redirect("/my-posts");
  } catch (error) {
    const message = error instanceof GqlRequestError ? error.message : "Could not create the post.";
